...
export default function CreateGlobalAgentPage() {
  ...
  useEffect(() => {
    ...
    if (!user.is_system_admin) {
      toast({ title: 'Access Denied', description: 'Only system administrators can create global agents', variant: 'destructive' ,duration: 2000  })
      navigate('/library')
      return
    }
  ...
  // ── Submit ──────────────────────────────────────────────────────────────
  const handleCreateGlobalAgent = async () => {
    ...
    try {
      ...
      const response = await apiClient.customAgents.marketplace.createGlobal(globalPayload)
      ...
      if (response.success && response.data) {
        ...
        toast({ 
          ...
          title: 'Success', 
          description: 'Global agent created and is now available in the marketplace.' ,
          duration: 2000
        })
        navigate('/library')
      }
      ...
      toast({ 
        ...
        title: 'Error', 
        description: error.message || 'Failed to create global agent', 
        variant: 'destructive' ,
        duration: 2000
      })
    } finally {
  ...
  // ── Render ──────────────────────────────────────────────────────────────
  return (
    <div className="flex flex-col bg-slate-50 dark:bg-[#0d1117] min-h-full">      <div className="flex-1">
      <div className="max-w-4xl mx-auto p-6">

        {/* ── Page header ── */}
        <div className="mb-6">
          <Button onClick={() => navigate('/library')} variant="ghost" className="mb-4">
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Marketplace
          </Button>
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Create Global Agent</h1>
          </div>
          <p className="text-gray-600 dark:text-gray-400">
            Create a global agent available to all users in the marketplace. This agent will have the "Official" badge.
          </p>
        </div>
        ...
        <Alert className="bg-teal-50 dark:bg-teal-900/20 border-teal-200 dark:border-teal-800 mb-6">
          ...
          <AlertDescription className="text-[#105e6e] dark:text-teal-300">
            System admin feature — global agents are visible to all users and cannot be modified by regular users.
          </AlertDescription>
        </Alert>

        {/* ── Agent Type selector (only on first step) ── */}
        {activeTab === 'basic' && (
          <div className="mb-6 p-4 border rounded-lg bg-gray-50 dark:bg-[#0d1117] dark:border-slate-800">
            <Label className="text-base font-semibold mb-3 block dark:text-white">Agent Type</Label>
            <div className="grid grid-cols-2 gap-4">
              {/* Chat */}
              <Card
                className={cn(
                  'p-4 cursor-pointer transition-all hover:shadow-md dark:bg-[#161b22] dark:border-slate-800',
                  agentFormData.agent_type === 'chat' && 'ring-2 ring-[#105e6e] bg-teal-50 dark:bg-teal-900/20'
                )}
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={cn(
                    ...
                    agentFormData.agent_type === 'chat' ? 'bg-[#105e6e] border-[#105e6e]' : 'border-gray-300 dark:border-gray-600'
                  )}>
                    {agentFormData.agent_type === 'chat' && <Check className="h-3 w-3 text-white" />}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold dark:text-white mb-1">Conversational Agent</h3>
              ...
              <Card
                className={cn(
                  'p-4 cursor-pointer transition-all hover:shadow-md dark:bg-[#161b22] dark:border-slate-800',
                  agentFormData.agent_type === 'flow' && 'ring-2 ring-[#105e6e] bg-teal-50 dark:bg-teal-900/20'
                )}
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={cn(
                    'w-5 h-5 rounded-full border-2 flex items-center justify-center mt-0.5',
                    agentFormData.agent_type === 'flow' ? 'bg-[#105e6e] border-[#105e6e]' : 'border-gray-300 dark:border-gray-600'
                  )}>
                    {agentFormData.agent_type === 'flow' && <Check className="h-3 w-3 text-white" />}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold dark:text-white mb-1">Flow Agent</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                ...
                <div key={step.key} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-1">
                    <div className={cn(
                      ...
                      isActive && 'bg-[#105e6e] border-[#105e6e] text-white',
                      !isDone && !isActive && 'bg-white dark:bg-[#0d1117] border-gray-300 dark:border-gray-700 text-gray-400 dark:text-gray-500'
                    )}>
                      {isDone ? <Check className="h-4 w-4" /> : idx + 1}
                    </div>
                    <span className={cn(
                      'text-xs mt-1 font-medium whitespace-nowrap',
                      isActive ? 'text-[#105e6e] dark:text-teal-400' : 'text-gray-400 dark:text-gray-500'
                    )}>
                      {step.label}
        ...
        {/* ── Basic Info ── */}
        {activeTab === 'basic' && (
          <Card className="p-6 dark:bg-[#161b22] dark:border-slate-800 space-y-5">
            <div>
              <Label htmlFor="global-agent-name" className="dark:text-white">
            ...
            <div>
              <Label htmlFor="global-agent-description" className="dark:text-white">Description</Label>
              <Textarea
                id="global-agent-description"
                placeholder="Describe what your global agent does..."
            ...
            {/* Initial Message */}
            <div>
              <Label htmlFor="global-agent-initial-message" className="dark:text-white">Initial Message</Label>
              <Textarea
                id="global-agent-initial-message"
        ...
        {activeTab === 'prompt' && (
          <Card className="p-6 dark:bg-[#161b22] dark:border-slate-800">
            <Label htmlFor="global-agent-prompt" className="dark:text-white">
        ...
        {/* ── Examples (optional) ── */}
        {activeTab === 'examples' && (
          <Card className="p-6 dark:bg-[#161b22] dark:border-slate-800 space-y-4">
            ...
            {agentFormData.example_queries.length > 0 && (
              <div className="space-y-2">
                {agentFormData.example_queries.map((example, index) => (
                  <Card key={index} className="p-4 dark:bg-[#161b22] dark:border-slate-800">
        ...
        {activeTab === 'workflows' && (
          <div className="space-y-4">
            {/* Design Your Workflow card - Exact match to Custom Agents */}
        ...
        <div className="flex justify-between items-center mt-8 pb-8">
          ...
          <div className="flex gap-2">
            {isFirstStep ? (
              <Button variant="outline" onClick={() => navigate('/library')} className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300">
                Cancel
              </Button>
            ) : (
              <Button variant="outline" onClick={goPrev} className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300">
                <ArrowLeft className="h-4 w-4 mr-2" />
                Previous
              </Button>
            )}
          ...
          <div>
            {isLastStep ? (
              <Button
                ...
                disabled={isSaving}
                className="bg-[#105e6e] hover:bg-[#0d4d59] text-white shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSaving ? (
                  <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating...</>
                ) : (
                  <><Sparkles className="mr-2 h-4 w-4" />Create Global Agent</>
                )}
              </Button>
            ) : (
              <Button onClick={goNext} className="bg-[#105e6e] hover:bg-[#0d4d59] text-white">
                Next
